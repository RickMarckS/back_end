...
lista_gatos = [
    ...
    Gato(8, "Oliver", "British Shorthair", 3),
...
for gato in lista_gatos:
    try:
        id = int(gato.id) if not isinstance(gato.id, int) else gato.id
        nome = str(gato.nome) if not isinstance(gato.nome, str) else gato.nome
        raca = str(gato.raca) if not isinstance(gato.raca, str) else gato.raca
        idade = int(gato.idade) if not isinstance(gato.idade, int) else gato.idade
        gato_tratado = Gato(id, nome, raca, idade)
        lista_gatos_tratada.append(gato_tratado)
    except Exception as e:
        raise e
    #Não consegui tratar a duplicata do id
    #Não consegui tratar a 2 lista então ajeitei ela de forma manual
